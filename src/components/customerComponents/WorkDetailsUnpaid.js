...
import { PAYMENT_DETAILS,CONFIRM_PAYMENT } from '../../GraphQL/customerMutations';
...
const WorkDetailsUnpaid=(props)=>{
    ...
    const [confirmPayment,ConfirmP]=useMutation(CONFIRM_PAYMENT);
    const [payment,{data,loading,error}]=useMutation(PAYMENT_DETAILS,{
        onCompleted: data =>{
            
            swal({
                title: "Thank You!",
                text: "Your Payment was Submitted!",
                icon: "success",
    ...
    const  payHerePay = () => {
        window.payhere.startPayment(payments);
    }

    window.payhere.onCompleted = function onCompleted(orderId) {
        console.log("Payment completed. OrderID:" + orderId);
        confirmPayment({variables:{appointment:props.appointment}})
        payment({variables:{from:props.Customer._id,
                to:props.SP_ID,
